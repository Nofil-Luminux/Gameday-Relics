    ...
    try {
      for (const item of items) {
          ...
          .insert([
            {
              ...
              amount: item.price,
              status: 'held',
              stripe_payment_id: `mock_${Date.now()}_${Math.random()}`
            }
          ]);
        ...
        const { error: updateError } = await supabase
          .from('orders')
          .update({ payment_status: 'escrowed', status: 'pending_shipment' })
          .eq('id', order.id);
        ...
        const { error: auditError } = await supabase
          ...
          .insert([
            {
              action: 'order_created_and_escrowed',
              order_id: order.id,
              user_id: user.id,
              metadata: { amount: item.price, product: item.title }
            }
          ]);
