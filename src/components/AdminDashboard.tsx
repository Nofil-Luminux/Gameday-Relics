...
export default function AdminDashboard({ isOpen, onClose }: AdminDashboardProps) {
  ...
  const handleReleaseEscrow = async (escrowId: string, orderId: string) => {
    try {
      const { error: updateEscrow } = await supabase
        .from('escrow_payments')
        .update({ status: 'released', released_at: new Date().toISOString() })
        .eq('id', escrowId);

      if (updateEscrow) throw updateEscrow;
      ...
      if (updateOrder) throw updateOrder;
  ...
  const handleResolveDispute = async (disputeId: string, orderId: string) => {
    ...
    try {
      const { error: updateDispute } = await supabase
        .from('disputes')
        .update({
          status: 'resolved',
          resolution,
      ...
      if (updateDispute) throw updateDispute;
